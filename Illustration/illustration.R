...
# setwd("/home/johannes/Documents/Ideas/armed_conflicts")
...
# define a grid of y values for plot etc
grid_y <- seq(from = -1, to = 1, by = 0.005)
...
# compute optimal choices under TADDA:
...
# grid:
grid_y_density <- seq(from = -2, to = 6, by = 0.01)
...
plot(grid_y_density, f, type = "l", ylab = "f(y)", xlab = "y", ylim = c(0, 0.4))
abline(v = mu, lty = 2, col = "darkgreen")
abline(v = med, lty = 3, col = "darkgreen")
abline(v = mu_modified, lty = 2, col = "darkorange")
abline(v = med_modified, lty = 3, col = "darkorange")
legend("topright", legend = c(paste0("Median: ", round(med, 2)),
                              paste0("Mean: ", round(mu, 2)), # hard-coded, modify as needed
                              expression(BA~under~TADDA[0]^(1): 0.10),
                              expression(BA~under~TADDA[0]^(2): 0.60)),
       lty = 3:2, bty = "n", col = c("darkgreen", "darkgreen", "darkorange", "darkorange"), cex = 0.9)
dev.off()
...
grid_y_hat <- seq(from = -1, to = 2, by = 0.01)
average_scores_ae <-
  average_scores_se <-
...
plot(grid_y_hat, average_scores_tadda_l1_0, type = "l", col = "black", xlab = expression(hat(y)),
     ylab = "expected score", xlim = c(-0.5, 1), ylim = yl)
...
text_in_box(med, 1.8, "m", col = "darkgrey")

abline(v = mu,  col = "darkgrey")
text_in_box(mu, 1.8, expression(mu), col = "darkgrey")

abline(v = 0, col = "darkgrey")
text_in_box(0, 1.65, 0, col = "darkgrey")

abline(v = epsilon,  col = "darkgrey")
text_in_box(epsilon, 1.8, expression(epsilon), col = "darkgrey")
...
plot(grid_y_hat, average_scores_tadda_l2_0, type = "l", col = "black", xlab = expression(hat(y)),
     ylab = "expected score", xlim = c(-0.5, 1), ylim = yl2)
...
abline(v = mu,  col = "darkgrey")
text_in_box(mu, 0.8*yl2[2], expression(mu), col = "darkgrey")

abline(v = 0, col = "darkgrey")
text_in_box(0, 0.65*yl2[2], 0, col = "darkgrey")

abline(v = epsilon,  col = "darkgrey")
text_in_box(epsilon, 0.8*yl2[2], expression(epsilon), col = "darkgrey")
...
library(xtable)
# scores:
xtable(expected_scores_small, 3, digits = 2)

# Bayes acts:
xtable(matrix(expected_scores[1:8, "value"], nrow = 1))
...
grid_epsilon <- c(seq(from = 0.01, to = 0.5, by = 0.02),
                  seq(from = 0.5, to = 4, by = 0.1))
...
x_labels <- c(0.01, 0.2, 0.5, 1, 2, 4)
y_labels <- c(0.01, 0.2, 0.5, 1)

# L1:
plot(sqrt(grid_epsilon), sqrt(bayes_acts_L1_v1), type = "l", col = "red", lty = "dashed",
     xlab = expression(epsilon), ylab = "Bayes act", ylim = sqrt(c(0, 1)), axes = FALSE)
...
text_in_box(0.17, 0.17, expression(epsilon), col = "darkgrey")
...
plot(grid_y, grid_TADDA_a, type = "l", xlab = expression(hat(y)),
     ylab = ylab, ylim = ylim, xlim = xlim)
mtext(expression(
  TADDA^(1)~with~y%in%paste("(", -epsilon, ",", epsilon, ")"~", varying"~hat(y))
), 3, cex = 0.9, line = 0.3)
...
box()
lines(grid_y, grid_TADDA_a)
lines(grid_y, grid_TADDA_v1_a - shift, col = "red", lty = "dashed")
...
plot(grid_y, grid_TADDA_b, type = "l", xlab = expression(hat(y)),
     ylab = "", ylim = ylim, xlim = xlim)
mtext(expression(TADDA^(1)~with~y > epsilon~", varying"~hat(y)), 3, cex = 0.9, line = 0.3)
rect(-epsilon, -1, epsilon, 11, col = "grey97", border = NA)
abline(h = 0:10, col = "grey95")
abline(v = -6:8, col = "grey95")
box()
lines(grid_y, grid_TADDA_b)
lines(grid_y, grid_TADDA_v1_b - shift, col = "red", lty = "dashed")
...
plot(grid_y, grid_TADDA_c, type = "l", xlab = expression(y),
     ylab = ylab, ylim = ylim, xlim = xlim)
mtext(
  expression(TADDA^(1)~with~hat(y)%in%paste("(", -epsilon, ",", epsilon, ")"~", varying"~y)),
  3, cex = 0.9, line = 0.3)
...
abline(v = -6:8, col = "grey95")
box()
lines(grid_y, grid_TADDA_c)
lines(grid_y, grid_TADDA_v1_c - shift, col = "red", lty = "dashed")
...
plot(grid_y, grid_TADDA_d, type = "l", xlab = expression(y),
     ylab = "", ylim = ylim, xlim = xlim)
mtext(expression(TADDA^(1)~with~hat(y) > epsilon~", varying"~y), 3, cex = 0.9, line = 0.3)
rect(-epsilon, -1, epsilon, 11, col = "grey97", border = NA)
abline(h = 0:10, col = "grey95")
abline(v = -6:8, col = "grey95")
box()
lines(grid_y, grid_TADDA_d)
lines(grid_y, grid_TADDA_v1_d - shift, col = "red", lty = "dashed")
lines(grid_y, grid_TADDA_v2_d + shift, col = "blue", lty = "twodash")
...
par(mar = c(0, 0, 0, 0))
plot(NULL, xlim = 0:1, ylim = 0:1, xlab = "", ylab = "", axes = FALSE)
legend("center", legend = c(expression({TADDA[0]^(1)}(hat(y), y)),
                            expression({TADDA1[epsilon]^(1)}(hat(y), y)),
                            expression({TADDA2[epsilon]^"(1)"}(hat(y), y))),
